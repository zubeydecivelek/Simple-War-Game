...
public class Goblin extends Zorde{
    ...
    private final int constantHP = 80;

    public Goblin(String name, int i, int j) {
        super(name, i, j);
    }
    ...
    public ArrayList<ArrayList<Integer>> controlEnemy(String[][] board) {
        ...
        for (int[] row : Commands.possibleMove) {
            int newI = getI() + row[0];
            int newJ = getJ() + row[1];
            if (0 <= newI && newI < board.length && 0 <= newJ && newJ < board.length && "HED".contains(board[newI][newJ].substring(0, 1))) {
